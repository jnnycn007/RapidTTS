...
UPPERCASE_LETTER_PRONUNCIATIONS = {
    "A": ["EY1"],
    "B": ["B", "IY1"],
    "C": ["S", "IY1"],
    "D": ["D", "IY1"],
...
class EnglishKernel(AbstractKernel):
    ...
    def g2p_en(self, pad_start_end=True, tokenized=None):
        ...
        for group in ph_groups:
            ...
            phone_len = 0
            word_len = len(group)
            if w in UPPERCASE_LETTER_PRONUNCIATIONS:
                phns, tns = refine_syllables([UPPERCASE_LETTER_PRONUNCIATIONS[w]])
                phones += phns
                tones += tns
                phone_len += len(phns)
            elif w.upper() in self.eng_dict:
